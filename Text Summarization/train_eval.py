...
def val_epoch(dataloader, model, criterion):
    ...
    for item in tqdm.tqdm(iter(dataloader),colour='green'):
        ...
        with torch.set_grad_enabled(False):
            ...
            loss_total += loss.item()*item['data'].size(0)
            bleu_score_curr, chrf_score_curr = evaluate_preds(decoder_outputs,target,idx2word)
    ...
    bleu_score_total = bleu_score_total/val_total
    chrf_score_total = chrf_score_total/val_total

    return loss_total, bleu_score_total, chrf_score_total
...
def train_val(train_loader, val_loader, model, optimizer, criterion, num_epochs):
    ...
    for epoch in tqdm.tqdm(range(num_epochs),colour='blue'):
        ...
        #### Validation
        val_loss_epoch, bleu_score_total, chrf_score_total = val_epoch(val_loader,model,criterion)
        ...
        print('Validation BLEU: '+str(bleu_score_total))
        print('Validation chrF: '+str(chrf_score_total))
