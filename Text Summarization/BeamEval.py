...
###### Dataloader
##### Dataset
X, y = preprocess(args.input_file)
...
if(args.model == 'lstm'):
    ...
    seq2seq_model.load_state_dict(torch.load(args.model_path))
...
##### Evaluation loop
def eval(dataloader, model, total):
    ...
    for item in tqdm.tqdm(iter(dataloader),colour='green'):
        ...
        with torch.set_grad_enabled(False):
            ...
            bleu_score_curr, chrf_score_curr, preds_curr = evaluate_preds(decoder_outputs,target,idx2word,True,args.beam_width)
    ...
    return bleu_score_total, chrf_score_total, predictions

bleu_score, chrf_score, predictions = eval(DataLoader,seq2seq_model,total)
print('BLEU: '+str(bleu_score))
print('chrF: '+str(chrf_score))
print(predictions)
